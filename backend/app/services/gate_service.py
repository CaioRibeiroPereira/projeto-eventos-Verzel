from app.core.qr import parse_ticket_payload, verify_ticket
...
class GateService:
    ...
    def validate(self, event_id: int, data: ValidateRequest, gate_user_id: int) -> ValidationResult:
        ...
        parsed = parse_ticket_payload(data.code)
        if not parsed:
            return ValidationResult(result="invalid", message="Código ilegível")

        ticket_id, signature = parsed
        if not verify_ticket(ticket_id, signature):
            return ValidationResult(result="invalid", message="Assinatura do QR não confere")

        ticket = self.repository.get_ticket(ticket_id)
        if not ticket:
            return ValidationResult(result="invalid", message="Ingresso não encontrado")
